...
router.post("/update-value", async (req, res) => {
    // 🛡️ Değişken isimlerinin Frontend'den gelenlerle (tagId, value) tam eşleştiğinden emin ol
    ...
    try {
        ...
        // 1️⃣ ADIM: Veritabanını kalıcı olarak güncelle
        // Not: Kolon adının 'initial_value' olduğundan emin ol
        ...
        if (dbResult.rowCount === 0) {
            console.error("❌ Tag bulunamadı, DB güncellenemedi.");
            return res.status(404).json({ error: "Tag not found in DB" });
        }

        console.log("✅ DB Updated:", dbResult.rows[0].tag_name);

        // 2️⃣ ADIM: Dashboard'a anlık bas (Burası Dashboard'daki rakamı değiştirir)
        if (io) {
            io.emit('liveData', { 
                sourceName: sourceName || 'VIRTUAL WORKSPACE', 
                tagName: tagName, 
                tagId: tagId,
        ...
        // 3️⃣ ADIM: Motorları tetikle (Hesaplamalar için)
        // FormulaEngine.process hem sanal tagleri hesaplar hem de results döner
        FormulaEngine.process({ tagId, value });

        // 4️⃣ ADIM: LogicEngine (Alarmlar ve Historian için)
