...
import { 
  ...
  Zap // 🎯 Enerji Analizörü için ikon eklendi
} from 'lucide-react';
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-10 animate-in fade-in duration-500 pb-20 px-4">
      ...
      <div className="flex justify-between items-end border-b border-slate-800/50 pb-8">
        ...
        <button onClick={() => setIsConnModalOpen(true)} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-2xl flex items-center gap-2 shadow-lg text-[10px] font-black uppercase tracking-widest transition-all">
              ...
              <div className="flex justify-between items-start mb-8">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className={`font-black text-2xl tracking-tight ${conn.enabled ? 'text-slate-100' : 'text-slate-600'}`}>{conn.name}</h3>
                      {isEnergy && <div className="p-1.5 bg-emerald-500/10 text-emerald-500 rounded-lg shadow-[0_0_10px_rgba(16,185,129,0.2)]"><Zap size={14} /></div>}
                    </div>
                    <p className="text-[10px] font-mono text-slate-500 truncate max-w-[180px]">{conn.endpoint_url}</p>
                    
                    <div className="flex items-center gap-2 mt-4">
                        <div className={`text-[9px] font-black px-3 py-1 rounded-lg border uppercase tracking-tighter transition-all duration-500 ${
                          isOnline ? 'text-emerald-400 border-emerald-500/30 bg-emerald-500/5' : 'text-red-400 border-red-500/30 bg-red-500/5'
                        }`}>
                          {isOnline ? 'Connection OK' : 'Source Offline'}
                        </div>
                        {isEnergy && (
                          <div className="text-[9px] font-black px-3 py-1 rounded-lg border border-blue-500/30 bg-blue-500/5 text-blue-400 uppercase tracking-tighter">
                            Energy Node
                          </div>
                        )}
                    </div>
                  ...
                  <div className="flex flex-col gap-3">
                    <button onClick={() => handleToggleEnabled(conn)} className={`p-3 rounded-2xl transition-all border ${conn.enabled ? 'bg-blue-600/10 text-blue-400 border-blue-500/20 hover:bg-blue-600/20' : 'bg-slate-800 text-slate-500 border-slate-700 hover:bg-slate-700'}`}>
          ...
          <div className="bg-slate-900 border border-slate-800 p-10 rounded-[3rem] w-full max-w-lg shadow-2xl">
            ...
            <form onSubmit={handleAddConnection} className="space-y-6">
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 flex items-center gap-4">
                <div className={`p-2 rounded-lg ${newConnData.connection_type === 'energy_analyzer' ? 'bg-emerald-500/20 text-emerald-500' : 'bg-slate-900 text-slate-600'}`}>
                  <Zap size={18} />
                </div>
                <div className="flex-1">
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Device Category</p>
                  <select 
                    className="w-full bg-transparent text-white font-bold outline-none text-xs"
                    value={newConnData.connection_type}
                    onChange={(e) => setNewConnData({...newConnData, connection_type: e.target.value})}
                  >
                    <option value="standard" className="bg-slate-900">Standard PLC / Device</option>
                    <option value="energy_analyzer" className="bg-slate-900 text-emerald-500">Professional Energy Analyzer</option>
                  </select>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[700] flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-slate-900 border border-slate-800 p-10 rounded-[3rem] w-full max-w-lg shadow-2xl">
            ...
            <form onSubmit={handleUpdateConnection} className="space-y-6">
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 flex items-center gap-4">
                <div className={`p-2 rounded-lg ${editConnData.connection_type === 'energy_analyzer' ? 'bg-emerald-500/20 text-emerald-500' : 'bg-slate-900 text-slate-600'}`}>
                  <Zap size={18} />
                </div>
                <div className="flex-1">
                  <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Device Category</p>
                  <select 
                    className="w-full bg-transparent text-white font-bold outline-none text-xs"
                    value={editConnData.connection_type}
                    onChange={(e) => setEditConnData({...editConnData, connection_type: e.target.value})}
                  >
                    <option value="standard" className="bg-slate-900">Standard PLC / Device</option>
                    <option value="energy_analyzer" className="bg-slate-900 text-emerald-500">Professional Energy Analyzer</option>
                  </select>
      ...
      {/* 🏷️ TAG MANAGEMENT MODAL (Aynı Kaldı) */}
      {isTagModalOpen && (
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-4xl max-h-[90vh] rounded-[3rem] shadow-2xl flex flex-col overflow-hidden">
            <div className="p-8 border-b border-slate-800 flex justify-between items-center bg-slate-950/50">
              <div className="flex items-center gap-3">
                ...
                {selectedConn?.connection_type === 'energy_analyzer' && <Zap size={20} className="text-emerald-500" />}
              </div>
              ...
              <form onSubmit={handleAddTag} className="bg-slate-950/50 p-6 rounded-3xl border border-slate-800 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input type="text" placeholder="e.g. Total Energy" required className="w-full bg-slate-900 border border-slate-800 rounded-xl p-3 text-xs text-white outline-none focus:border-blue-500" 
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="kWh" className="w-full bg-slate-900 border border-slate-800 rounded-xl p-3 text-xs text-white outline-none focus:border-blue-500" 
