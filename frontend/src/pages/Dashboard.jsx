...
import { 
  ...
  ArrowUp, ArrowDown, Server, Link2
} from 'lucide-react';
import { api } from '../services/api';
import EnergyAnalyzerWidget from '../components/EnergyAnalyzerWidget'; 

// --- 🕹️ SETPOINT CONTROLLER SUB-COMPONENT (KORUNDU) ---
  ...
  return (
    <div className="space-y-6 mt-4 animate-in fade-in duration-500">
      <div className="relative group">
        <input type="number" value={isNaN(inputValue) ? "" : inputValue} onChange={(e) => { const val = parseFloat(e.target.value); setInputValue(isNaN(val) ? 0 : val); }} className="w-full bg-slate-950 border-4 border-slate-800 rounded-[2.5rem] p-8 text-center text-6xl font-black text-amber-500 italic font-mono outline-none focus:border-amber-600/50 transition-all shadow-inner" />
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-600 text-white text-[9px] font-black px-4 py-1 rounded-full uppercase tracking-widest shadow-xl">Control Value</div>
      </div>
      <div className="grid grid-cols-2 gap-4">
          <button type="button" onClick={() => setInputValue(prev => parseFloat(((prev || 0) - 1).toFixed(2)))} className="py-5 bg-slate-800 rounded-2xl text-white font-black hover:bg-slate-700 transition-all shadow-lg active:scale-95 flex items-center justify-center border-b-4 border-slate-950"> <ArrowDown size={20}/> </button>
          <button type="button" onClick={() => setInputValue(prev => parseFloat(((prev || 0) + 1).toFixed(2)))} className="py-5 bg-slate-800 rounded-2xl text-white font-black hover:bg-slate-700 transition-all shadow-lg active:scale-95 flex items-center justify-center border-b-4 border-slate-950"> <ArrowUp size={20}/> </button>
      </div>
      <button type="button" onClick={handleUpdate} className="w-full py-6 bg-amber-600 hover:bg-amber-500 text-white font-black uppercase text-[11px] tracking-[0.3em] rounded-[1.5rem] shadow-2xl transition-all flex items-center justify-center gap-3 active:scale-95 group">
...
// --- 🏷️ UNIFIED TAG SELECTOR (KORUNDU) ---
const TagSelector = ({ label, value, onChange, groupedTags, searchTerm }) => (
  <div className="space-y-2">
    ...
    <div className="relative">
        <select className="w-full bg-slate-950 border-2 border-slate-800 rounded-2xl p-4 pl-5 text-white text-[12px] font-semibold focus:border-[#009999] outline-none appearance-none cursor-pointer uppercase tracking-tight shadow-inner transition-all" value={value} onChange={e => onChange(e.target.value)}>
        ...
        <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-700"> <Search size={16} /> </div>
    </div>
  </div>
);
...
const Dashboard = ({ liveData = {}, connections = [], userId }) => {
  ...
  // --- 🏛️ 1. SYSTEM OVERVIEW CALCULATIONS (YENİ) ---
  ...
  // --- 🚀 2. SMART AUTO-MAPPING FOR ENERGY (YENİ) ---
      ...
      case 'numeric':
        return (
          <div className="flex items-baseline gap-4 mt-4">
            <span className="text-8xl font-black tracking-tighter text-[#00ffcc] font-mono italic drop-shadow-[0_10px_20px_rgba(0,255,204,0.15)]">{currentVal.toFixed(2)}</span>
        ...
        return (
          <div className="relative flex items-center justify-center h-44 mt-4">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
              ...
              <circle cx="50" cy="50" r={radius} stroke="currentColor" strokeWidth="10" fill="transparent" strokeDasharray={circumference} strokeDashoffset={offset} className="text-[#009999] transition-all duration-1000" strokeLinecap="round" />
            </svg>
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-12 pb-20 px-6 pt-10 text-white font-['Inter',_sans-serif]">
      ...
      {/* 🏛️ YENİ: SYSTEM OVERVIEW HEADER */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-in slide-in-from-top-10 duration-700">
        {systemStats.map((stat, i) => (
          <div key={i} className="bg-slate-900/40 p-8 rounded-[2.5rem] border-2 border-slate-800 shadow-2xl relative overflow-hidden group hover:border-[#009999]/40 transition-all duration-500">
            <div className={`absolute top-0 right-0 p-6 opacity-5 group-hover:opacity-10 transition-opacity ${stat.color}`}>{stat.icon}</div>
            <div className="space-y-1">
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic flex items-center gap-2">{stat.label}</p>
              <h4 className={`text-4xl font-black italic tracking-tighter ${stat.color}`}>{stat.val}</h4>
              <div className="flex items-center gap-2 mt-2">
                <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${stat.color.replace('text-', 'bg-')}`}></div>
                <p className="text-[9px] font-bold text-slate-600 uppercase tracking-wider">{stat.sub}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start gap-10 border-b-2 border-slate-800 pb-12 text-white">
        <div className="space-y-1 min-w-[350px]">
          <div className="flex items-center gap-2 mb-2"><div className="w-10 h-1 bg-[#00ffcc]"></div><span className="text-[#00ffcc] text-[10px] font-bold uppercase tracking-[0.4em]">Real-Time Core</span></div>
          ...
          <div className="flex gap-4 mt-8">
            <button onClick={toggleEditMode} className={`px-8 py-4 rounded-xl flex items-center gap-3 transition-all font-bold text-[11px] uppercase tracking-widest border-2 shadow-2xl ${isEditMode ? 'bg-[#009999] text-white border-[#00ffcc]/30' : 'bg-slate-900 text-slate-500 border-slate-800 hover:border-[#009999]/30 hover:text-[#00ffcc]'}`}> {isEditMode ? <Save size={18} /> : <Settings2 size={18} />} {isEditMode ? 'Lock Protocol' : 'Customize UI'} </button>
            {isEditMode && <button onClick={() => setIsAddModalOpen(true)} className="bg-[#009999] hover:bg-[#00cccc] text-white px-8 py-4 rounded-xl flex items-center gap-3 shadow-2xl text-[11px] font-bold uppercase tracking-widest transition-all"> <Plus size={20} /> Add Widget </button>}
          </div>
        </div>
      ...
      {/* 🎛️ DASHBOARD GRID (KORUNDU) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 pt-4">
        {widgets.map((w) => (
          <div key={w.id} className={`group bg-slate-900/30 border-2 rounded-[3rem] p-10 shadow-2xl relative transition-all duration-500 overflow-hidden hover:border-[#009999]/30 hover:bg-slate-900/50 ${w.type === 'energy_analyzer' ? 'lg:col-span-2 border-[#009999]/20' : w.type === 'setpoint' ? 'border-amber-600/30 shadow-amber-900/10' : 'border-slate-800'}`}>
            {isEditMode && <button onClick={() => removeWidget(w.id)} className="absolute top-6 right-6 p-3 bg-red-600/10 text-red-500 rounded-2xl z-20 border border-red-500/20 shadow-xl transition-all hover:bg-red-600 hover:text-white"><X size={18} /></button>}
            ...
            {w.type !== 'energy_analyzer' && (
              <h2 className="text-slate-500 text-[11px] mb-8 flex items-center gap-3 uppercase font-bold tracking-[0.3em] italic group-hover:text-[#00ffcc] transition-colors">
                <div className={`p-2 bg-slate-950 rounded-lg shadow-inner ${w.type === 'gauge' || w.type === 'setpoint' ? 'text-amber-500' : 'text-[#009999]'}`}> {w.type === 'numeric' && <Cpu size={14} />} {w.type === 'gauge' && <GaugeIcon size={14} />} {w.type === 'sparkline' && <LineChart size={14} />} {w.type === 'setpoint' && <Settings2 size={14} />} </div>
      ...
      {/* 📂 DEPLOY MODAL (SMART MAPPING İLE GÜNCELLENDİ) */}
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-slate-950/98 backdrop-blur-3xl flex items-center justify-center z-[500] p-6 animate-in fade-in duration-300">
          <div className="bg-[#0b1117] border-2 border-slate-800 p-12 rounded-[4rem] w-full max-w-3xl shadow-[0_0_100px_rgba(0,0,0,0.8)]">
            <div className="flex items-center gap-5 mb-12">
                <div className="w-16 h-16 bg-[#009999]/20 text-[#00ffcc] rounded-[2rem] flex items-center justify-center shadow-inner border border-[#009999]/20"><DashboardIcon size={32}/></div>
                <div><h3 className="text-4xl font-bold text-white uppercase italic tracking-tighter">Deploy New Logic</h3><p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em] mt-1 italic">Infrastructure Visualization Engine</p></div>
            </div>
            ...
            <div className="space-y-10">
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-5 gap-4">
                  {[
                    ...
                    { id: 'setpoint', icon: <Settings2 size={24} className="text-amber-500"/>, label: 'Setpoint' },
              ...
              <div className="bg-slate-950/50 p-10 rounded-[3rem] border-2 border-slate-800/50 shadow-inner max-h-[300px] overflow-y-auto scrollbar-hide">
                 {newWidget.type === 'energy_analyzer' ? (
                   /* ⚡ AKILLI ENERJİ SEÇİMİ (CONNECTION BAZLI) */
                   <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-300 text-white">
                      <label className="text-[10px] text-slate-500 uppercase font-black tracking-widest italic flex items-center gap-2"> <Server size={14} className="text-[#00ffcc]"/> SELECT ENERGY ANALYZER DEVICE </label>
                      <select 
                        className="w-full bg-slate-900 border-2 border-slate-800 rounded-2xl p-5 text-white font-bold outline-none focus:border-[#00ffcc] uppercase tracking-wider cursor-pointer"
                      ...
                      <div className="grid grid-cols-2 gap-3 opacity-40 italic text-[9px] text-slate-500 font-bold uppercase p-2">
                        <p>⚡ Volt Mapping: {newWidget.config.voltageId ? '✅' : '❌'}</p><p>⚡ Curr Mapping: {newWidget.config.currentId ? '✅' : '❌'}</p>
                        <p>⚡ Power Mapping: {newWidget.config.powerId ? '✅' : '❌'}</p><p>⚡ Energy Mapping: {newWidget.config.energyId ? '✅' : '❌'}</p>
                      </div>
                   </div>
                 ) : (
